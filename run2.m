...
X = []
for i = length(t):-1:1
    if length(0:dx:LT(i)-dx) < length(0:dx:LT(end)-dx)
        cn = length(0:dx:LT(end)-dx) -length(0:dx:LT(i)-dx) 
        c = zeros(1,cn)
        xx = [0:dx:LT(i)-dx LT(i) c]
    else
        xx = [0:dx:LT(i)-dx LT(i)]
    end
    X = [xx;X]
end
...
%step 1 z(t)
z = Z_eq1(N,alfa,t,k,X,dx,init,dt,Nt,N1,v_or)

%step 2 HJB equation
nu = HJB1(N,alfa,beta,eta,rho,t,k,X,dx,init,dt,Nt,N1,v_or,z,T,z0,p0)
...
M = M_upwind1(init,X,t,N1,Nt,v_star,dx,dt)


%step5
v_new = Int1(t,X,dx,M,v_star)
